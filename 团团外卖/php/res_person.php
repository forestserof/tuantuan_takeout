...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["form_type"]) && $_POST["form_type"] == "update_info") {
    ...
    // 执行更新个人信息的SQL语句
    $updateQuery = "UPDATE merchants SET merchantName = '$name' WHERE merchantID = $merchantID";
    $updateResult = mysqli_query($connection, $updateQuery);

    if ($updateResult) {
        // 个人信息更新成功，进行相应处理
        header("Location: ../html/商家个人主页.php");
        exit();
    } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["form_type"]) && $_POST["form_type"] == "update_password") {
    $resID = $_SESSION['merchantID'];
    $oldPassword = $_POST['old_password'];
    $newPassword = $_POST['new_password'];

    // 查询当前用户的密码
    $selectQuery = "SELECT password FROM merchants WHERE merchantID = $resID";
    ...
    if ($selectResult && mysqli_num_rows($selectResult) === 1) {
        ...
        if ($oldPassword === $currentPassword) {
            // 旧密码正确，执行更新密码的SQL语句
            $updateQuery = "UPDATE merchants SET password = '$newPassword' WHERE merchantID = $resID";
            $updateResult = mysqli_query($connection, $updateQuery);

            if ($updateResult) {
                // 密码更新成功，进行相应处理
                echo '<script>alert("密码修改成功");window.location.href="../html/商家个人主页.php";</script>';
                exit();
            } else {
            ...
            // 旧密码不匹配，显示弹窗提示
            echo '<script>alert("旧密码输入不正确");window.location.href="../html/商家个人主页.php";</script>';
            exit();
        }
    } else {
